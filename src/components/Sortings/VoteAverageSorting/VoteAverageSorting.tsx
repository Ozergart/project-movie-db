...
const VoteAverageSorting: FC<IProps> = ({setQuery,query}) => {
    ...
    const descClick =()=>{
        ...
        setQuery(prev => {prev.set("sort_by","vote_average.asc")
        return prev
        } )
    }
    const ascClick =()=>{
        setStatus(statuses.desc)
        setQuery(prev=> {prev.set("sort_by", "vote_average.desc")
        return prev
        })
    }
    ...
    return (
        <div className={css.cont}>
            {status===statuses.neutral?<div onClick={neutralClick} className={css.filter}>середньою оцінкою </div>:null}
            {status ===statuses.desc?<div onClick={descClick} className={css.filter}>середньою оцінкою
                <img width="15" height="15"
                     src="https://img.icons8.com/ios-filled/50/expand-arrow--v1.png"
                     alt="expand-arrow--v1"/></div>:null}
            {status === statuses.asc?<div onClick={ascClick} className={css.filter}>середньою оцінкою
